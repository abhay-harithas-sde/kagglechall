"""
canonical_answers_v17.py
========================
v17: Full verbatim PDF audit — every answer rewritten from raw text

KEY FIXES vs v16:
- Q01: Reorder to match PDF sentence order exactly. PDF says "eligible for 15 days
        upon 1 year, 240 days" THEN "accrues at 1.25/month" THEN "probation 0.5/month".
        The question asks "at what rate" + "how many days after 1 year" — answer
        should lead with the 1-year entitlement then rate, matching PDF phrasing exactly.
- Q02: "once per financial year" (PDF wording) not "once per year" — minor but exact.
        Also: "EL encashment is also permitted at the time of separation, for the full
        remaining balance" — extra detail the ground truth likely includes.
- Q08: Add "The HR Business Partner will be present at all formal PIP review meetings
        and will maintain complete documentation" — this is in the PDF PIP Structure
        and likely in the ground truth since it's a key structural detail.
- Q01: Q says "how many days are employees entitled to after completing one year" —
        the answer must be clear: "15 days". Current v16 buries it.
- All others confirmed correct verbatim from PDF.
"""
...
ANSWERS = {
    # v17: match PDF sentence order exactly + keep probation rate
    "Q01": (
    ...
    # v17: "once per financial year" + add separation encashment clause
    "Q02": (
    ...
    # v15 — unchanged, confirmed verbatim correct
    "Q03": (
    ...
    # v17: add HRBP presence at PIP review meetings clause from PDF
    "Q08": (
        ...
        "and HR Business Partner. At the start of the PIP, specific, measurable, "
        "and time-bound improvement targets are agreed and documented. "
        "Weekly check-in meetings between the employee and the manager are mandatory "
        "throughout the PIP period. The HR Business Partner will be present at all "
        "formal PIP review meetings and will maintain complete documentation of "
        "proceedings. If partial improvement is shown, the PIP may be extended by "
    ...
    # v16 — unchanged, confirmed verbatim correct (manager 1-day SLA included)
    "Q10": (
