"""
gen_submission.py — Generate submission.csv from canonical answers
Usage: python gen_submission.py [version_number]
e.g.:  python gen_submission.py 15

Uses the canonical Cell 15 question ciphertexts from the official starter notebook
so the grader can verify question identity correctly.
"""
import csv, json, sys, importlib
from pathlib import Path
from cryptography.fernet import Fernet

version = sys.argv[1] if len(sys.argv) > 1 else "15"
...
# These are the authoritative ciphertexts the grader uses to verify questions.
_Q = [
...
# Decrypt questions from canonical ciphertexts
questions = {
    qid: fernet.decrypt(enc.encode()).decode()
    for qid, enc in _Q
}

QIDs = [q[0] for q in _Q]
...
# Also write a human-readable JSON for review
preview = [
    {"question_id": q, "question": questions[q], "answer": ANSWERS[q]}
    for q in QIDs
]
Path(__file__).parent.joinpath(f"answers_v{version}.json").write_text(
...
print()
for q in QIDs:
    tag = "🚫" if ANSWERS[q].startswith("I'm sorry") else "✅"
    print(f"  {tag} {q}: {ANSWERS[q][:80].replace(chr(10), ' | ')}")
