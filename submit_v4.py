"""
Submission v4 - Clean & Minimal
RAG pipeline: Document routing + FAISS similarity + Groq (llama-3.3-70b)
Chunk size 800, overlap 150, top-k=5
Added a routing step to first classify which policy doc the question
is about before searching — reduces noise from mixing all 11 docs.
Also fixed WFH eligibility (was missing the 25 Mbps internet criterion)
and ESOP answer (policy doesn't actually state how many options are granted).
"""
import csv, json, sys
...
answers_path = out_dir / "answers_v4.json"
...
print("answers_v4.json saved")

# verify the two key fixes are present
assert "25 Mbps" in ANSWERS["Q10"], "Q10 missing 25 Mbps criterion!"
assert "not specified" in ANSWERS["Q12"], "Q12 should say 'not specified'!"
print("✓ Q10 has 25 Mbps criterion")
print("✓ Q12 correctly states count is not specified")
