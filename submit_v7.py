...
submit_v7.py
============
Submission v7 — Verbatim PDF re-alignment pass.

Key fixes over v6:
- Q01: Sentence ORDER matches PDF exactly (eligibility sentence first, then
        "Thereafter, Earned Leave accrues at the rate of 1.25 days per month").
        v6 had the order reversed.
- Q04: Reverted back to exact PDF phrasing: "more than 2 consecutive days"
        (v6 incorrectly changed this to "3 or more consecutive days").
        The PDF says: "Sick Leave taken for more than 2 consecutive days requires
        a Medical Certificate..."
- Q08: Owner clarified — "at the joint discretion of HR and the manager"
        (verbatim from PDF Partial improvement bullet).
- Q09: Activity descriptions now mirror PDF table exactly, including
        "collected from peers and subordinates", "submitted on ZyroHR portal",
        "submits draft rating", "HR and L7+ Leaders" as owner for Stage 4.
- Q10: Criterion 6 is now verbatim from PDF:
        ...
        WFH type descriptions also taken verbatim from the PDF table.
"""
import csv
import json
import sys
...
from canonical_answers_v7 import ANSWERS
...
answers_path = out_dir / "answers_v7.json"
...
print(f"✅ answers_v7.json saved")

# ── Print diff vs v6 for review ───────────────────────────────────────────────
import importlib.util, sys as _sys
v6_spec = importlib.util.spec_from_file_location(
    "canonical_answers_v6",
    str(out_dir / "canonical_answers.py"),
)
v6_mod = importlib.util.module_from_spec(v6_spec)
v6_spec.loader.exec_module(v6_mod)
V6_ANSWERS = v6_mod.ANSWERS

print("\n" + "=" * 60)
print("DIFF: v7 vs v6 (changed questions)")
print("=" * 60)
changed = False
for qid, _ in _Q:
    old = V6_ANSWERS.get(qid, "")
    new = ANSWERS[qid]
    if old.strip() != new.strip():
        changed = True
        print(f"\n[{qid}]")
        print(f"  v6: {old[:200].replace(chr(10), ' | ')}")
        print(f"  v7: {new[:200].replace(chr(10), ' | ')}")
if not changed:
    print("  (no changes — answers are identical)")
...
print(f"\n✅ submission.csv generated — {len(rows)} rows")
print("\nAnswers summary:")
for r in rows:
    ans = ANSWERS[r["question_id"]]
    print(f"  {r['question_id']}: {ans[:90].replace(chr(10), ' | ')}...")
